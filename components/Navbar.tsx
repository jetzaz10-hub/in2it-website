          ...
          {navLinks.map((link) => (
            ...
            >
              <a
                href={link.href}
                className={`
                  flex items-center gap-1.5 px-4 py-2 text-[13px] font-bold text-white/80 hover:text-white transition-colors uppercase tracking-wider
                `}
              >
                {link.label}
                {link.hasDropdown && (
                    ...
                    <div className="bg-gray-900/90 backdrop-blur-2xl border border-gray-700 rounded-2xl p-2 shadow-2xl">
                      {link.items.map((item) => (
                        <button
                          ...
                          className="w-full text-left px-4 py-2.5 text-xs font-medium text-white/70 hover:bg-white/5 hover:text-white rounded-lg transition-all"
                        >
                          {item.label}
                        </button>
                      ))}
